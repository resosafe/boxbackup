...
void BackupQueries::CommandList(const std::vector<std::string> &args, const bool *opts)
{
	...
	if(args.size() > 0)
	{
		...
		if ( opts['i']) {
			rootDir=::strtoll(args[0].c_str(), 0, 16);
			...
			rootDir = FindDirectoryObjectID(storeDirEncoded,
				opts[LIST_OPTION_ALLOWOLD] || opts[LIST_OPTION_BY_ID],
				opts[LIST_OPTION_ALLOWDELETED] || opts[LIST_OPTION_BY_ID], 
				opts[LIST_OPTION_BY_ID]);
			}
			if ( rootDir == 0) {
...
int64_t BackupQueries::FindDirectoryObjectID(const std::string &rDirName,
	bool AllowOldVersion, bool AllowDeletedDirs, bool ByIDs,
	...
	for(unsigned int e = 0; e < dirElements.size(); ++e)
	{
		if(dirElements[e].size() > 0)
		{
			...
			else
			{
				...
				if ( ByIDs ) {
					en = dir.FindEntryByID(::strtoll(dirElements[e].c_str(), 0, 16));
				} else {
					BackupStoreDirectory::Iterator i(dir);
					BackupStoreFilenameClear dirname(dirElements[e]);
					en = i.FindMatchingClearName(dirname);
				}
...
void BackupQueries::CommandChangeDir(const std::vector<std::string> &args, const bool *opts)
{
	...
	int64_t id = FindDirectoryObjectID(dirName, opts['o'] || opts['i'], opts['d'] || opts['i'], opts['i'],
			&newStack);
