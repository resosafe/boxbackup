...
int BackupStoreAccountsControl::SetOptions(int32_t ID, int32_t Options, bool fix, bool force)
{
	std::string rootDir;
	int discSetNum;
	...
	if( fix )
	{
		...
		int flags = HousekeepStoreAccount::FixForSnapshotMode;
		if(force)
		{
			flags |= HousekeepStoreAccount::Force;
		}
