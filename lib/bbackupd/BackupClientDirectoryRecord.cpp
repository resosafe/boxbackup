	...
	const std::vector<std::string> &rDirs)
{
		...
		d != rDirs.end(); ++d)
	{
		...
		if((en != 0) && !(en->IsDir()))
		{
			...
			// Get rid of it.
			BackupProtocolCallable &connection(rContext.GetConnection());
			connection.QueryDeleteFile(mObjectID /* in directory */, storeFilename, 0, rBackupLocation.mDoNotKeepDeletedFiles);
	...
	const std::string &rFilename)
{
	...
	BackupProtocolCallable &connection(rParams.mrContext.GetConnection());
	connection.QueryDeleteDirectory(pEntry->GetObjectID(), 0, true);
